...
from analyze import analyze_stock, format_minute_analysis, format_realtime

PORTFOLIO_FILE = (
    Path.home() / ".openclaw" / "skills" / "a-stock-analysis" / "portfolio.json"
)
...
def show_portfolio():
    ...
    for pos in positions:
        ...
        print(
            f"{pos.get('name', 'N/A'):<10} {pos['code']:<8} {pos['cost']:<8.3f} {pos['quantity']:<8} {cost_value:<12.0f}"
        )
...
def update_position(
    code: str, cost: float = None, quantity: int = None, name: str = None
):
    """更新持仓"""
    portfolio = load_portfolio()
    positions = portfolio.get("positions", [])
...
def analyze_portfolio():
    ...
    print(
        f"  总盈亏: {total_value - total_cost:+.0f} 元 ({(total_value / total_cost - 1) * 100:+.2f}%)"
    )
