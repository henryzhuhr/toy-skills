...
class StockAnalyser:
    ...
    def fetch_eastmoney_news(
        ...
        params = {"input": name, "type": 14, "count": limit}
        try:
    ...
    def fetch_dragon_tiger(self, date: Optional[str] = None) -> List[Dict]:
        """获取龙虎榜数据"""
        if not date:
    ...
    def analyze_gold_correlation(self, gold_price: float, stocks: List[Dict]) -> str:
        ...
        for stock in stocks:
            ...
            if corr in ["强正相关", "中等正相关"]:
                analysis.append(
                    f"📈 {stock['name']}: 与金价{corr}，金价上涨可能带动该股"
                )
    ...
    def generate_insight(self, stock: Dict, price_data: Dict, alerts: List) -> str:
        ...
        # 2. 资金流向
        self.fetch_fund_flow(code, stock.get("market", "sz"))
